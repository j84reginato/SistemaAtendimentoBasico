...
# Redefine o diretório inicial do usuário Vagrant
if ! grep -q "cd /var/www" /home/vagrant/.profile; then
    echo "cd /var/www" >> /home/vagrant/.profile
fi

echo "** Execute o seguinte comando para instalar dependências:"
echo "    vagrant ssh -c 'composer install'"
echo "** Visite http://localhost:8080 em seu navegador para visualizar o aplicativo. **"
SCRIPT
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    vb.customize ["modifyvm", :id, "--name", "Sistema de Atendimento Basico - Ubuntu 16.04"]
  end
end
